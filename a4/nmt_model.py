...
class NMT(nn.Module):
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ## split approach
        # for enc_hiddens_proj_i in torch.split(enc_hiddens_proj, 1, dim=1):
        #     enc_shape = enc_hiddens_proj_i.size()
        #     enc_hiddens_proj_i = enc_hiddens_proj_i.view(enc_shape[0], enc_shape[2], enc_shape[1])
        #     e_i = torch.bmm(dec_hidden, enc_hiddens_proj_i)
        #     e_i = torch.squeeze(e_i, dim=2)
        #     e_i = torch.squeeze(e_i, dim=1)
        #     e_t.append(e_i)
        # e_t = torch.stack(e_t)
        # e_t = e_t.view(e_t.size()[1],e_t.size()[0])
