...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            tmp_tup = (self.stack[-1], self.stack[-2])
            del self.stack[-2]
            self.dependencies.append(tmp_tup)
        elif transition == "RA":
            tmp_tup = (self.stack[-2], self.stack[-1])
            del self.stack[-1]
            self.dependencies.append(tmp_tup)
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
        if len(unfinished_parses)> batch_size:
            ...
            current_size = len(unfinished_parses)
        next_transitions = model.predict(mini_batch)
        for i in range(current_size):
            mini_batch[i].parse_step(next_transitions[i])
            if not mini_batch[i].buffer and len(mini_batch[i].stack) == 1:
                del_idx = unfinished_parses.index(mini_batch[i])
                unfinished_parses = unfinished_parses[:del_idx] + unfinished_parses[del_idx+1:]
